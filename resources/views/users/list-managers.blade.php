                ...
                <tbody id="managersTableBody">
                    @forelse($managers as $manager)
                        <tr class="border-b hover:bg-gray-50"
                            data-fname="{{ strtolower($manager->fname) }}"
                            data-lname="{{ strtolower($manager->lname) }}"
                            data-email="{{ strtolower($manager->email) }}"
                            data-contact="{{ $manager->contact_number ?? '' }}">
            ...
            if (noResults && rows.length > 0) {
                if (visibleCount === 0 && searchTerm !== '') {
                    ...
                    noResults.querySelector('td').innerHTML = `
                        <div class="flex flex-col items-center">
                            <p class="text-lg">No managers found matching "${searchTerm}"</p>
                        </div>
                    `;
                } else {

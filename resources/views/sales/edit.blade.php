        ...
        <div id="products-wrapper" class="space-y-2">
            @if($sale->details->count() > 0)
                @foreach($sale->details as $index => $detail)
                    @php
                        // Find the original quantity deducted (stored in Quantity field which is actually Kilo)
                        $originalQuantityDeducted = $detail->Quantity;
                    @endphp
            <div class="flex gap-2 items-center product-row">
                <select name="products[{{ $index }}][Product_ID]" class="flex-1 border p-2 rounded focus:ring-2 focus:ring-green-500 product-select" required>
                    ...
                    @foreach($products as $product)
                        @php
                            ...
                            // CRITICAL FIX: Show available_stock (which includes the quantity from this sale)
                            $displayStock = $product->available_stock ?? $product->Quantity_in_Stock;
                        ...
                        <option value="{{ $product->Product_ID }}" 
                            ...
                            {{ $product->Product_Name }}{{ $varietyText }} (Stock: {{ $displayStock }})
                ...
                <!-- FIXED: Quantity shows the actual quantity, NOT Kilo -->
                <input type="number" name="products[{{ $index }}][Quantity]" 
                       value="{{ $originalQuantityDeducted }}" 
                       placeholder="Quantity" 
                       class="w-24 border p-2 rounded focus:ring-2 focus:ring-green-500 quantity-input" 
                       min="1" step="1" required>

                <!-- Kilo (For Pricing) -->
                <input type="number" name="products[{{ $index }}][Kilo]" 
                       value="{{ $detail->Quantity }}" 
                       placeholder="Kilo" 
                       class="w-24 border p-2 rounded focus:ring-2 focus:ring-green-500 kilo-input" 
                ...
                <input type="number" name="products[{{ $index }}][Price]" 
                       ...
                       class="w-28 border p-2 rounded focus:ring-2 focus:ring-green-500 price-input bg-gray-100" 
                       min="0" step="0.01" readonly required>

                <!-- Total -->
                <span class="w-24 text-gray-700 font-semibold total-display">{{ number_format($detail->Quantity * $detail->unit_price, 2) }}</span>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let productIndex = {{ $sale->details->count() > 0 ? $sale->details->count() : 1 }};
    ...
    saleForm.addEventListener('submit', (e) => {
        ...
        wrapper.querySelectorAll('.product-row').forEach((row, index) => {
            ...
            if (!quantityInput.value || !Number.isInteger(quantity) || quantity < 1) {
                errors.push(`${productName}: Quantity must be a whole number and at least 1`);
                quantityInput.classList.add('border-red-500', 'bg-red-50');
